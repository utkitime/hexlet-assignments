...
public class GuestsController {
    ...
    @PostMapping
    public ResponseEntity<GuestCreateDTO> registerGuest(@Valid @RequestBody GuestCreateDTO guestCreateDTO) {
        var guest = guestMapper.map(guestCreateDTO);
        guestRepository.save(guest);
        return new ResponseEntity<>(guestCreateDTO, HttpStatus.CREATED);
    }
}
